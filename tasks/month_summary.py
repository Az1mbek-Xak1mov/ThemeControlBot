...
async def create_activity_report(filename="monthly_report.xlsx"):
    wb = Workbook()
    ...
    for idx, group in enumerate(groups):
        ...
        total = await total_messages(30, group.chat_id)
        for info in info_user:
            count = await get_messages_for_chat(30, group.chat_id, info.chat_id, )
            if total!=0:
...
async def send_summary_to_admin_month():
    filepath = await create_activity_report()
    file = FSInputFile(filepath)
    await bot.send_document(admin_chat_id, file, caption="📈 Oylik hisobot")
    await bot.session.close()


if __name__ == "__main__":
    asyncio.run(send_summary_to_admin_month())
